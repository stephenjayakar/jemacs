...
import { FUNDAMENTAL_MODE, inferMode } from "../../src/kernel/buffer"
...
test("inferMode routes conf-family files", () => {
  ...
  expect(inferMode("app.cfg")).toBe("conf-mode")
  expect(inferMode("tool/config")).toBe("conf-mode")
  expect(inferMode("etc/nginx.conf.bak")).toBe("conf-mode")
})

test("inferMode guesses a conf sub-mode from file contents", () => {
  expect(inferMode(".ssh/config", "Host foo\n  HostName example.com\n  User me\n")).toBe("conf-space-mode")
  expect(inferMode("app.conf", "# comment\n[main]\nkey = value\nother = 2\n")).toBe("conf-unix-mode")
  expect(inferMode("app.conf", "; comment\n[main]\nkey = value\n")).toBe("conf-windows-mode")
  expect(inferMode("app.conf", "key: value\nother: 2\n")).toBe("conf-colon-mode")
  expect(inferMode("app.conf", "// comment\n// comment\n// comment\n// comment\n// comment\n")).toBe("conf-javaprop-mode")
  expect(inferMode("app.conf", "")).toBe("conf-mode")
})
...
test("inferMode routes text suffixes to text-mode and everything else to fundamental-mode", () => {
  // GNU Emacs `auto-mode-alist` sends only these suffixes to `text-mode`.
  expect(inferMode("notes.txt")).toBe("text")
  expect(inferMode("notes.text")).toBe("text")
  expect(inferMode("draft.article")).toBe("text")
  expect(inferMode("draft.letter")).toBe("text")
  // Everything unmatched stays in fundamental-mode, as in Emacs.
  expect(inferMode("LICENSE")).toBe(FUNDAMENTAL_MODE)
  expect(inferMode("README")).toBe(FUNDAMENTAL_MODE)
  expect(inferMode("foo.bar")).toBe(FUNDAMENTAL_MODE)
})
