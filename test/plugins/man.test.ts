...
test("install registers man commands and man-mode bindings", () => {
  ...
  expect(editor.commands.get("man-follow")).toBeDefined()
  expect(editor.commands.get("Man-update-manpage")).toBeDefined()
  ...
  expect(mode?.keymap?.get("return")).toBe("man-follow")
  expect(mode?.keymap?.get("g")).toBe("Man-update-manpage")
})
...
test("RET on a foo(1) reference opens that page through the backend", async () => {
  ...
  expect(fed.command).toBe("man-follow")
