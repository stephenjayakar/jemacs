...
test("magit bisect commands, keymap, and transient are wired", async () => {
  ...
  expect(editor.minibufferCompletionDisplay?.text).toContain("Bisect")
  expect(editor.minibufferCompletionDisplay?.text).toContain("s        start")
  expect(editor.minibufferCompletionDisplay?.text).toContain("g        good")
  expect(editor.minibufferCompletionDisplay?.text).toContain("b        bad")
  expect(editor.minibufferCompletionDisplay?.text).toContain("k        skip")
  expect(editor.minibufferCompletionDisplay?.text).toContain("r        reset")
})
