...
describe("rectangle-mark-mode", () => {
  ...
  test("backspace and C-d delete the region-rectangle and end the mode", async () => {
    for (const command of ["delete-backward-char", "delete-char"]) {
      const editor = makeEditor()
      install(editor)
      const buffer = editor.scratch(`*rect-del-${command}*`, TEXT, "text")
      buffer.point = 1

      await editor.run("rectangle-mark-mode")
      buffer.point = TEXT.indexOf("gamma") + 4 // col 4 on line 3

      await editor.run(command)
      expect(buffer.text).toBe("aa one\nb two\nga three\n")
      expect(editor.isMinorModeEnabled("rectangle-mark-mode", buffer)).toBe(false)
    }
  })

  test("backspace deletes one character when the rectangle is empty", async () => {
    const editor = makeEditor()
    install(editor)
    const buffer = editor.scratch("*rect-del-empty*", TEXT, "text")
    buffer.point = 3

    await editor.run("rectangle-mark-mode")
    await editor.run("delete-backward-char")

    expect(buffer.text).toBe("alha one\nbeta two\ngamma three\n")
  })
