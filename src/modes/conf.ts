...
export function installConfMode(): void {
  ...
  defineMode({ name: "conf-colon-mode", parent: "conf-mode", commentStart: "#", fontLock: confColonFontLock })
  defineMode({ name: "conf-javaprop-mode", parent: "conf-mode", commentStart: "#", fontLock: confFontLock })
}
...
export function confSpaceFontLock(buffer: BufferModel, range?: FontLockRange): TextSpan[] {
  // Space-separated conf files assign with ` `, so `=`/`:` are ordinary value
  // characters (e.g. ssh's `ProxyCommand ssh -W %h:%p bastion`).
  return confFontLockWithComments(buffer, ["#"], true, range, [])
}

export function confColonFontLock(buffer: BufferModel, range?: FontLockRange): TextSpan[] {
  return confFontLockWithComments(buffer, ["#"], false, range, [":"])
}
...
function confFontLockWithComments(buffer: BufferModel, commentChars: string[], spaceSeparated: boolean, range?: FontLockRange, separators: string[] = ["=", ":"]): TextSpan[] {
  ...
  for (const line of textLines(text)) {
    ...
    const separator = firstSeparator(content, separators)
    if (separator !== -1) {
...
function firstSeparator(text: string, separators: string[]): number {
  let found = -1
